 ...
 * utility, in org.openmrs.test.jupiter.BaseContextSensitiveTest, replace
 * Containers.ensureDatabaseRunning() with DbUtil.ensureDatabaseCreated() and then run: mvn test
 * -DuseInMemoryDatabase=false If you do not want to use the defaults, you can use the optional
 * command line options below: -DdatabaseName -DdatabaseUsername -DdatabasePassword -DdatabaseUrl
 * The default values are: database username=root, database password=test, database
 * name=openmrs_test, database host=localhost and database port=3306 If you want to override the
 * default database connection url, you would need to use %s for the database name. To use
 * PostgreSQL, you would need -Ddatabase=postgres and then the default database port would change to
 * 5432
 */
public class DbUtil {
